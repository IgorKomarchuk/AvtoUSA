...
export default async function AutopostingPage() {
  ...
  return (
    <AdminShell title="Автопублікації" description="Керування якістю лотів, каналами, чергою та історією публікацій.">
      ...
      <div className={`mt-4 rounded-2xl border p-4 text-sm ${data.workerOnline ? "border-emerald-400/15 bg-emerald-400/[.06] text-emerald-200" : "border-rose-400/15 bg-rose-400/[.06] text-rose-200"}`}>
        <strong>{data.workerOnline ? "Автоматичний воркер працює" : "Автоматичний воркер не підтверджено"}</strong>
        <p className="mt-1 text-xs opacity-70">Останній сигнал: {data.workerHeartbeatAt ? formatDateTime(data.workerHeartbeatAt) : "ще не отримано"}. Нормальний інтервал — до 1 хвилини.</p>
      </div>
