  ...
  return <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[.035] p-5 sm:p-6">
      ...
      <form onSubmit={add} className="mt-5 grid gap-4">
        <label className="grid gap-2 text-sm text-white/60">Назва<input value={name} onChange={(event) => setName(event.target.value)} required minLength={2} maxLength={60} placeholder="Наприклад, Основний акаунт" className="premium-focus min-h-12 rounded-xl border border-white/10 bg-black/25 px-4 text-slate-900"/></label>
        <label className="grid gap-2 text-sm text-white/60">API key<input type="password" autoComplete="new-password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} required minLength={20} maxLength={300} placeholder="ak_live_••••••••" className="premium-focus min-h-12 rounded-xl border border-white/10 bg-black/25 px-4 font-mono text-slate-900"/></label>
