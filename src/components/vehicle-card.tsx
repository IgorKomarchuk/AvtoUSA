...
export function VehicleCard({ vehicle, priority = false, sourceChannel }: { vehicle: VehicleData; priority?: boolean; sourceChannel?: SocialChannel }) {
  ...
  return <>
    <article role="button" tabIndex={0} onClick={openDialog} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openDialog();}}} className="premium-focus group flex h-full cursor-pointer flex-col overflow-hidden rounded-[26px] border border-white/10 bg-[#101210] text-left shadow-[0_24px_70px_rgba(0,0,0,.22)] transition duration-300 hover:-translate-y-1.5 hover:border-[#ff6b00]/50" aria-label={`Дізнатися деталі про ${vehicle.title}`}>
      <div className="relative block aspect-[16/10] overflow-hidden bg-[#161816]"><Image src={image} alt={vehicle.photos[0]?.alt ?? vehicle.title} fill priority={priority} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition duration-700 group-hover:scale-[1.045]"/><div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/90 to-transparent"/><div className="absolute left-3 top-3 flex flex-wrap gap-2"><Badge className={vehicle.isDemo?"border-amber-400/40 bg-amber-500/90 text-black":"border-emerald-300/30 bg-emerald-500/85"}>{vehicle.isDemo?"DEMO":"LIVE"}</Badge><Badge>{vehicle.platform}</Badge>{freshness&&<Badge className="border-sky-300/30 bg-sky-500/85">{freshness}</Badge>}</div></div>
    ...
    {open&&<div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/80 p-3 backdrop-blur-md sm:p-6" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setOpen(false)}}><section role="dialog" aria-modal="true" aria-labelledby={`lead-${vehicle.id}`} className="relative w-full max-w-2xl overflow-hidden rounded-[30px] border border-white/15 bg-[#101210] shadow-[0_30px_100px_rgba(0,0,0,.7)]"><button type="button" onClick={()=>setOpen(false)} className="premium-focus absolute right-4 top-4 z-10 grid size-11 place-items-center rounded-full border border-white/15 bg-black/60" aria-label="Закрити"><X size={20}/></button><div className="grid sm:grid-cols-[.8fr_1.2fr]"><div className="relative min-h-52 bg-black"><Image src={image} alt={vehicle.title} fill sizes="(max-width: 640px) 100vw, 40vw" className="object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent"/><div className="absolute inset-x-0 bottom-0 p-5"><span className="text-xs font-black uppercase tracking-[.12em] text-[#ff7b1a]">{vehicle.platform} · Lot #{vehicle.lotNumber}</span><h2 id={`lead-${vehicle.id}`} className="mt-2 text-2xl font-bold leading-tight">{vehicle.title}</h2><p className="mt-2 text-lg font-black">{formatUsd(vehicle.buyNowPrice??vehicle.currentBid)}</p></div></div><div className="p-5 pt-16 sm:p-7 sm:pt-16"><p className="text-xs font-black uppercase tracking-[.13em] text-[#ff7b1a]">Дізнатися деталі</p><h3 className="mt-2 text-2xl font-bold tracking-[-.04em]">Залиште номер — ми все перевіримо</h3><p className="mt-3 text-sm leading-6 text-white/50">Менеджер уточнить стан лота, витрати та зв’яжеться з вами найближчим часом.</p><div className="mt-5"><LeadForm compact vehicle={{vehicleId:vehicle.id,vin:vehicle.vin,lotNumber:vehicle.lotNumber,vehicleTitle:vehicle.title,price:formatUsd(vehicle.buyNowPrice??vehicle.currentBid),vehicleUrl:detailUrl,source:"vehicle_card"}}/></div></div></div></section></div>}
  </>;
}
