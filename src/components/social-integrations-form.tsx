...
type Props = { configured: Record<SocialChannel, boolean>; masks: Record<SocialCredentialField, string | null>; checks: Partial<Record<SocialChannel, SocialIntegrationCheck>> };
...
export function SocialIntegrationsForm({ configured, masks, checks }: Props) {
  ...
  return <form className="mt-6 grid gap-4" onSubmit={async(event)=>{event.preventDefault();const form=event.currentTarget;const payload=Object.fromEntries(Array.from(new FormData(form).entries()).map(([key,value])=>[key,String(value)]));if(await run({action:"save",...payload})){form.reset();location.reload();}}}>
    <div className="grid gap-4 xl:grid-cols-2">
      <section className="rounded-3xl border border-white/10 bg-white/[.035] p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold">Telegram</h2><Status active={configured.TELEGRAM}/></div><div className="mt-5 grid gap-4"><Field label="Bot token" name="telegramBotToken" mask={masks.telegramBotToken} secret hint="Створюється у @BotFather. Значення ніколи не показується повністю."/><Field label="Channel ID або @username для автопостів" name="telegramChannelId" mask={masks.telegramChannelId} hint="Бот має бути адміністратором публічного каналу."/><Field label="Chat ID менеджера для заявок" name="telegramLeadChatId" mask={masks.telegramLeadChatId} hint="Особистий або груповий Chat ID, куди надходитимуть ім’я, телефон і дані авто."/></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={()=>run({action:"test",channel:"TELEGRAM"})} className="rounded-xl border border-white/10 px-4 py-2 text-sm font-bold hover:bg-white/[.06]">Перевірити з’єднання</button><button type="button" disabled={busy} onClick={()=>window.confirm("Надіслати позначений TEST-пост з реальним авто у Telegram?")&&run({action:"test_publication",channel:"TELEGRAM"})} className="rounded-xl bg-[#ff6b00] px-4 py-2 text-sm font-bold">Надіслати TEST-пост</button></div><CheckResult check={checks.TELEGRAM}/></section>
