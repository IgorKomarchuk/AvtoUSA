...
export function SiteHeaderClient({ contacts }: { contacts: SiteContacts }) {
  ...
  return <><header className="sticky top-0 z-50 border-b border-white/[.07] bg-[#070807]/85 backdrop-blur-2xl">
    ...
    {!open&&!contactsOpen&&<button type="button" onClick={openContacts} className="premium-focus fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-3 z-[110] flex min-h-12 items-center gap-2 rounded-full border border-white/20 bg-[#ff6b00] px-4 text-sm font-black text-white shadow-[0_14px_40px_rgba(0,0,0,.5),0_8px_24px_rgba(255,107,0,.3)] md:hidden" aria-label={copy.contacts}><Phone size={19}/><span>{copy.contacts}</span></button>}
