...
export function GoogleSettingsForm({ settings }: { settings: GoogleTrackingSettings }) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-6">
      ...
      <section className="rounded-3xl border border-emerald-300/15 bg-emerald-400/[.045] p-5 text-sm text-white/60 sm:p-7"><h2 className="font-bold text-white">Події, які передаються</h2><ul className="mt-3 grid gap-2 sm:grid-cols-2"><li>page_view — перегляд сторінки</li><li>view_item — перегляд автомобіля</li><li>generate_lead — успішна заявка</li><li>click_phone — клік на телефон</li></ul><p className="mt-4 text-xs leading-5 text-white/40">Ім’я, телефон, email та інші персональні дані в Google не надсилаються. UTM-параметри зберігаються разом із заявкою в адмінці.</p></section>
